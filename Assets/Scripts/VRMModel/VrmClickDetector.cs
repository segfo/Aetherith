...
public class VrmClickDetector : MonoBehaviour
{
    ...
    public void Ready()
    {
        ...
        this.characterController = characterController.GetComponent<CharacterController>();
        vrmLoader = characterController.GetComponent<VRMLoader>();
        var chatManager = GameObject.Find("ChatManager");
        if (chatManager == null) { Debug.LogError("致命的なエラー: 必須コンポーネントのChatManagerがヒエラルキーにありません"); }
        this.chatManager = chatManager.GetComponent<ChatManager>();
    ...
    private void OnFootDoubleClicked(Transform footTransform)
    {
        ...
        if (footTransform.IsChildOf(rightFoot))
        {
            ...
            if (!AppConfigManager.Instance.Config.MetamorphoseEnabled || vrmListCont == 1|| chatManager.InitializeState != ChatManagerInitializeState.Ready)
            {
